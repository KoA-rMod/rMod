...
#ifndef __ATOMIC_SDK_ATOMIC_BINARYDATA_H_INCLUDED__
#define __ATOMIC_SDK_ATOMIC_BINARYDATA_H_INCLUDED__
...
namespace Atomic
{
    ...
    public:
        ...
        static uint32_t PackBitsBE(uint8_t* data, uint64_t value, uint32_t byteOffset, uint32_t bitOffset, const uint8_t len)
        {
            ...
            if (len + bitOffset <= 8)
            {
                const auto dataPointer = (uint8_t*)&data[byteOffset];
                const auto bitmaskUC = (uint8_t)bitmask;
                const auto valueUC = (uint8_t)value;
                *dataPointer &= bitmaskUC;
                *dataPointer |= valueUC;
            }
            else if (len + bitOffset <= 16)
            {
                const auto dataPointer = (uint16_t*)&data[byteOffset];
                const auto bitmaskUC = (uint16_t)bitmask;
                const auto valueUC = (uint16_t)value;
                *dataPointer &= bitmaskUC;
                *dataPointer |= valueUC;
            }
            else if (len + bitOffset <= 32)
            {
                const auto dataPointer = (uint32_t*)&data[byteOffset];
                const auto bitmaskUC = (uint32_t)bitmask;
                const auto valueUC = (uint32_t)value;
                *dataPointer &= bitmaskUC;
                *dataPointer |= valueUC;
            }
            else if (len + bitOffset <= 64)
            {
                const auto dataPointer = (uint64_t*)&data[byteOffset];
                *dataPointer &= bitmask;
                *dataPointer |= value;
            }
            else
         ...
         * Unpacks data from the given buffer. (Little Endian)
         */
        static uint64_t UnpackBitsLE(uint8_t* data, uint32_t byteOffset, uint32_t bitOffset, uint8_t const len)
        {
            byteOffset += bitOffset >> 3;
            bitOffset %= 8;
            ...
            if (bytesNeeded == 1)
            {
                const uint8_t bitmask = 0xFF >> bitOffset;
                retVal = (modifieddata[0] & bitmask) >> (8 - (len + bitOffset));
            }
            else
            {
                const int32_t newBitOffset = (bytesNeeded * 8) - (bitOffset + len);
...
#endif // __ATOMIC_SDK_ATOMIC_BINARYDATA_H_INCLUDED__